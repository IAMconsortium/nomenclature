...
class CodeList(BaseModel):
    ...
    @classmethod
    def check_forbidden_characters(cls, v: Dict[str, Code]) -> Dict[str, Code]:
        """Check that no forbidden characters and/or stray tags are left in codes after tag replacement"""
        forbidden = ["{", "}", '"']
        ...
        def _check_string(value):
            if isinstance(value, str):
                if any(char in value for char in forbidden):
                    raise ValueError(
                        f"Unexpected forbidden character in codelist: {code.name}."
                        " Check for quotes and/or if tags were spelled correctly."
                    )
